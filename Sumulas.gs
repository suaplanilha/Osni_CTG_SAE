...
function getPartidasSumula_() {
  ...
  return dbRead('tb_partidas').map(partida => Object.assign({}, partida, {
    ...
    ctgA: equipes[partida.id_equipe_a] ? equipes[partida.id_equipe_a].ctg_responsavel : '',
    equipeB: equipes[partida.id_equipe_b] ? equipes[partida.id_equipe_b].nome_equipe : 'Equipe B',
    ctgB: equipes[partida.id_equipe_b] ? equipes[partida.id_equipe_b].ctg_responsavel : ''
  }));
}
...
function renderTruco_(p, logo) {
  ...
  return `<article class="sumula truco">${cabecalhoSumula_(titulo, logo, true)}<div class="versus"><span>MTG/Federação: ${escapeHtml_(p.ctgA)}</span><b>X</b><span>MTG/Federação: ${escapeHtml_(p.ctgB)}</span></div><table><thead><tr><th>NOME DO TRIO</th><th>Nº DO TRIO</th><th>TENTOS</th><th>PARTIDAS</th><th>TENTOS</th><th>Nº DO TRIO</th><th>NOME DO TRIO</th></tr></thead><tbody>${rows}<tr><th colspan="3">SOMA: ${score_(p.placar_a)}</th><th>X</th><th colspan="3">SOMA: ${score_(p.placar_b)}</th></tr></tbody></table><p class="note">Obs.: melhor de três partidas, conforme regulamento da modalidade.</p>${resultadoDuplo_(p, 'TRIO')}<div class="local">Local e Data: ${dataPartida_(p)}</div>${assinaturas_(['Nome do Árbitro/Assinatura', 'Responsável p/Dep. de Truco', 'Diretor de Esportes da CBTG'])}</article>`;
}

function renderTava_(p, logo) {
  const cells = Array.from({ length: 10 }, (_, i) => `<th>${i + 1}</th>`).join('');
  const rows = Array.from({ length: 4 }, (_, i) => `<tr><th>${i + 1}</th><td class="athlete">${i === 0 ? escapeHtml_(p.equipeA + ' X ' + p.equipeB) : ''}</td>${'<td></td>'.repeat(10)}<td>${i === 0 ? score_(p.placar_a) + ' x ' + score_(p.placar_b) : ''}</td></tr>`).join('');
  return `<article class="sumula tava">${cabecalhoSumula_('SÚMULA TAVA', logo, true)}<div class="mini-id"><b>Nº ${escapeHtml_(p.chave || '')}</b><b>Equipe: ${escapeHtml_(p.ctgA)} x ${escapeHtml_(p.ctgB)}</b></div><table><thead><tr><th colspan="2">ATLETAS</th><th colspan="10">ARREMESSOS / PONTOS</th><th>TOTAL</th></tr><tr><th></th><th>NOME</th>${cells}<th>PONTOS</th></tr></thead><tbody>${rows}<tr><th colspan="2">Anotações possíveis</th><th>2</th><th>1</th><th>0</th><th>-1</th><th>-2</th><th colspan="5">TOTAL GERAL</th><th>${score_(p.placar_a)} x ${score_(p.placar_b)}</th></tr></tbody></table>${assinaturas_(['Árbitro', 'Capitão', 'Dir. Esporte'])}</article>`;
}
